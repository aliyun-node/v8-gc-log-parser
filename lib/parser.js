...
const EXTERNAL_MEMORY_GLOBAL = types.EXTERNAL_MEMORY_GLOBAL;
...
const READ_ONLY_SPACE = types.READ_ONLY_SPACE;
const NEW_LARGE_OBJECT_SPACE = types.NEW_LARGE_OBJECT_SPACE;
const CODE_LARGE_OBJECT_SPACE = types.CODE_LARGE_OBJECT_SPACE;
...
const keys = {
  ...
  [NEW_LARGE_OBJECT_SPACE]: 'New large object space',
  [CODE_LARGE_OBJECT_SPACE]: 'Code large object space'
};
...
class GCLParser {
  ...
  _VerboseTrace(source, trace) {
    ...
    if (this._VerboseTraceStart(source, data)
      ...
      let mark2 = this.lexer.markPosition();
      if (!this._VerboseTraceExternalGlobal(source, trace)) {
        this.lexer.restorePosition(mark2);
      } else {
        this.lexer.skipLine();
      }

      if (!this._VerboseTraceEnd(source, data)) {
        this.lexer.restorePosition(mark);
        return false;
      }
  ...
  _HeapSection(keyword) {
    return this._KeyWord(NEW_LARGE_OBJECT_SPACE, keyword) // v12.x
      || this._KeyWord(CODE_LARGE_OBJECT_SPACE, keyword)
      // v10.x
      || this._KeyWord(READ_ONLY_SPACE, keyword)
      // v4.x/6.x
      || this._KeyWord(NEW_SPACE, keyword)
  ...
  // Source 'External memory global' Number KB
  _VerboseTraceExternalGlobal(source, trace) {
    let mark = this.lexer.markPosition();
    if (this._Source(source, trace) && this._KeyWord(EXTERNAL_MEMORY_GLOBAL)) {
      let value = {};
      if (this._Number(value) && this._KeyWord(KB)) {
        trace.external_memory_global = value.data;
        return true;
      }
    }
    this.lexer.restorePosition(mark);
  ...
  // Source 'Total time spent in GC' ':' Number MS
  _VerboseTraceEnd(source, trace) {
    let mark = this.lexer.markPosition();
    if (this._Source(source) && this._KeyWord(TOTAL_TIME)) {
      let value = {};
